...
<display:table class="table-default" export="false" id="obj" name="${ComNotaItensList}" requestURI="/nota-itens">
	...
	<display:column style="width: 150px;text-align: center;">
		<a class="btn-xs" data-toggle="modal" href="<c:url value="/nota-itens/${obj.id}"/>"><span class="icon-pencil"></span>&nbsp;<fmt:message key="button.editar"/></a>
		<a class="btn-xs" href="<c:url value="/nota-itens/${obj.id}"/>" onclick="deletar(this);"><span class="icon-trash"></span>&nbsp;<fmt:message key="button.deletar"/></a>
	</display:column>
</display:table>