...
<form action="<c:url value="/financeiro/conta-bancaria"/>" class="form-modern" id="formlistarfincontabancaria" method="get" name="formlistarfincontabancaria" role="search">
...
<display:table class="table-default" export="false" id="obj" name="${FinContaBancariaList}" requestURI="/financeiro/conta-bancaria">
	...
	<display:column style="text-align: center;width: 150px;">
		...
		<button class="btn-der btn-xs" formaction="<c:url value="/financeiro/conta-bancaria?obj.id=${obj.id}"/>" onclick="deletar(this);" type="button"><span class="icon-trash"></span>&nbsp;<fmt:message key="button.deletar"/></button>
	</display:column>
</display:table>
