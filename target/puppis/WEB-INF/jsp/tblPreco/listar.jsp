...
<nav class="o-navbar--gary" role="complementary">
	<div class="o-breadcrumb--arrow">
		...
		<span class="o-breadcrumb__link"><a class="is-inactive" href="<c:url value="/tabela-de-preco"/>" role="link" title="<fmt:message key="nav.cadastro.tabela.preco"/>"><fmt:message key="nav.cadastro.tabela.preco"/></a></span>
		<span class="o-breadcrumb__link"><a class="is-inactive" href="<c:url value="/tabela-de-preco"/>" role="link" title="<fmt:message key="nav.lista"/>"><fmt:message key="nav.lista"/></a></span>
	</div>
	<form action="<c:url value="/tabela-de-preco"/>" class="o-form" id="formtblprecolistar" method="get" name="formtblprecolistar" role="search">
...
<display:table class="c-table--lube c-table--hovered c-table--zebrered c-table--bordered" export="false" id="obj" name="${TblPrecoList}" requestURI="/tabela-de-preco">
	...
	<display:column style="text-align: center;width: 185px;">
		<a class="o-button--low o-button--small" href="<c:url value="/tabela-de-preco/${obj.id}"/>" role="link" title="<fmt:message key="button.editar"/>"><i class="icon-pencil"></i>&nbsp;<fmt:message key="button.editar"/></a>
		<button class="o-button--der o-button--small" formaction="<c:url value="/tabela-de-preco?obj.id=${obj.id}"/>" onclick="DialogController.build(event, this, requestDelete, 'Deseja confirmar a exclusao', 'icon-trash');" role="button" type="button" title="<fmt:message key="button.deletar"/>"><i class="icon-trash"></i>&nbsp;<fmt:message key="button.deletar"/></button>
	</display:column>
</display:table>
