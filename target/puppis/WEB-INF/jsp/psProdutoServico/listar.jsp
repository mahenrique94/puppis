...
<nav class="o-navbar--gary" role="complementary">
	<div class="o-breadcrumb--arrow">
		...
		<span class="o-breadcrumb__link"><a class="is-inactive" href="<c:url value="/produto-servico"/>" role="link" title="<fmt:message key="nav.cadastro.produto.servico"/>"><fmt:message key="nav.cadastro.produto.servico"/></a></span>
		<span class="o-breadcrumb__link"><a class="is-inactive" href="<c:url value="/produto-servico"/>" role="link" title="<fmt:message key="nav.lista"/>"><fmt:message key="nav.lista"/></a></span>
	</div>
	<form action="<c:url value="/produto-servico"/>" class="o-form" id="formpsprodutoservicolistar" method="get" name="formpsprodutoservicolistar" role="search">
...
<display:table class="c-table--lube c-table--hovered c-table--zebrered c-table--bordered" export="false" id="obj" name="${PsProdutoServicoList}" requestURI="/produto-servico">
	...
	<display:column style="text-align: center;width: 185px;">
		<a class="o-button--low o-button--small" href="<c:url value="/produto-servico/${obj.id}"/>" role="link" title="<fmt:message key="button.editar"/>"><span class="icon-pencil"></span>&nbsp;<fmt:message key="button.editar"/></a>
		<button class="o-button--der o-button--small" formaction="<c:url value="/produto-servico?obj.id=${obj.id}"/>" onclick="DialogController.build(event, this, requestDelete, 'Deseja confirmar a exclusao', 'icon-trash');" role="button" type="button" title="<fmt:message key="button.deletar"/>"><i class="icon-trash"></i>&nbsp;<fmt:message key="button.deletar"/></button>
	</display:column>
</display:table>
